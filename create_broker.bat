...
docker run -d --name %container_name%  --cap-add=ALL -w /work_space/Broker -it stream_network

REM Connect the container to the "streaming_network" network and assign an IP address
docker network connect --ip=172.22.0.2 streaming_network %container_name%
