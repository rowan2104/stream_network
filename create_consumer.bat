...
set "container_base_name=consumer"
...
docker run -d --name %container_name%  --cap-add=ALL -w /work_space/Consumer -it stream_network

REM Connect the container to the "streaming_network" network and assign an IP address
docker network connect --ip=172.22.2.%container_index% streaming_network %container_name%
